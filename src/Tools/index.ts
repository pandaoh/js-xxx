 ...
 * @LastEditTime: 2024-08-12 14:09:02
 * @Description: 工具函数
 ...
 * @returns
 */
export function getFileNameFromUrl(url: string, onlyExt = false): string {
  const ext = url.match(/[^\/]*\.(\w+)(?:\?.*)?$/)?.[1] || 'txt';
  if (onlyExt) {
    return ext;
  }
  const filenameOfUrl = url.match(/([^/]*?)\.\w+(\?.*?)?$/)?.[1];
  const filename = filenameOfUrl ? safeDecodeURI(filenameOfUrl) : String(Date.now());
  return `${filename}.${ext}`;
}
