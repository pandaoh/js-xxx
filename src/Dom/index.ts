 ...
 * @LastEditTime: 2025-02-24 14:23:53
...
export function createClickLogListener(callback?: any): any {
  ...
  function handleClick(event: any) {
    ...
    if (isOrder && orderKey) {
      ...
      callback &&
        callback(event, newLogKey, { trigger: trigger ?? 'click', params, logKey, userAgent: navigator?.userAgent });
      return;
    }
    ...
    if (maxSequence === undefined) {
      ...
      callback &&
        callback(event, logKey, { trigger: trigger ?? 'click', params, logKey, userAgent: navigator?.userAgent });
      return;
    }
